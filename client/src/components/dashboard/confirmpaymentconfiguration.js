    ...
    this.setState({role:currentUser.role})
      ...
      headers: { Authorization: cookie.load('token') },
        ...
        (res)=>{
          ...
          if(currentUser && currentUser.role!=null && currentUser.role=="Expert" && res && res!=null && res.data && res.data!=null && res.data.user && res.data.user!=null && res.data.user!=undefined && mainrole=="Expert" && mainId==""){
            	this.setState({errorMessage:"Your Payment Configuration Is Not Set. Please Configure It To Access Session Page"})
          }
          else if(currentUser && currentUser.role!=null && currentUser.role=="Expert" && res && res!=null && res.data && res.data!=null && res.data.user && res.data.user!=null && res.data.user!=undefined && mainrole=="Expert" && mainId!=""){
          	// console.log(res)

          		self.setState({successMessage:"You have already filled the required information",errorMessage:"",info:"You will be redirected to dashboard."})
                  var sec=4
          	...
          	this.setState({errorMessage:"You are not authorized to view this page", info:"You will be redirected to dashboard."})
                  var sec=4
                  var timer = setInterval(function(){
                    self.setState({info:"Redirecting in "+ sec +" Seconds."})
                    sec= sec-1
	              ...
	              <div className="row row-offcanvas row-offcanvas-left">
	                ...
	                <div className="column col-sm-9 col-xs-11" id="main">
						...
						{ this.state.errorMessage && this.state.errorMessage!="" &&
					  	...
					  	{ this.state.errorMessage && this.state.errorMessage!="" && this.state.role=="Expert" &&
...
export default connect(null, {protectedTest })(ConfirmPaymentConfiguration);
