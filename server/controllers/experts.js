...
exports.userExpertUpdate = async function(req, res, next){
  var user_obj;
  await User.findOne({email:req.body.user_email},function(error,result){
    if(error){
      console.log(error);
    }else{
      user_obj = result;
  ...
  if(req.body.file){
    ...
    if (req.body.file.split(',')[0].includes('png')==true){
      ...
      fs.writeFile("../client/src/public/profile_images/"+filename, base64Data, 'base64', function(err,data) {
        if(err){
          console.log(err);
        }
        console.log(data);
  ...
  var firstName = req.body.updated_name.split(' ')[0] != 'undefined' ? req.body.updated_name.split(' ')[0] : '';
  var lastName = req.body.updated_name.split(' ')[1] != 'undefined' ? req.body.updated_name.split(' ')[1] : '';
 ...
 if(filename!=""){
   ...
   User.findOneAndUpdate({_id:user_obj._id}, updateuser, {new:true}, function(err4, company_obj){
    if (err4) {
...
exports.getExpertsSubCategoryList = function (req, res, next) {
...
exports.upload = function (req, res, next) {
  console.log('save profile image admin----');
  ...
  if(req.body.base64_image){
    ...
    if (req.body.base64_image.split(',')[0].includes('png')==true){
      ...
      filename = name+'.'+extension;
      fs.writeFile("../client/src/public/profile_images/"+filename, base64Data, 'base64', function(err) {
        console.log(err);
      });
    } 
   ...
   if(filename!=""){
     ...
     User.findOneAndUpdate({_id:req.body.user_id}, updateuser, {new:true}, function(err4, user_obj){
      if (err4) {
        console.log('error occured');
        console.log(err4);
        return next(err4);
