...
exports.getComments = (req, res) => {
  ...
  Comment.aggregate([
    ...
    {$sort: {createdAt: -1}}
        ...
        comments.forEach(async function(val){
          var like_slugs = [];
          var dislike_slugs = [];
          val.voters.forEach(function(val2){
            like_slugs.push(val2.slug);
          })
          if(val.voters_dislikes){
            val.voters_dislikes.forEach(function(val3){
...
exports.dislikeComment = (req, res) => {
  ...
  return Comment.findById(id, (error, comment) => {
    ...
    var is_found_2 = false;
    comment.voters_dislikes.forEach(function(item){
      if(item.slug==author){
        is_found_2 = true;
      }
    })
